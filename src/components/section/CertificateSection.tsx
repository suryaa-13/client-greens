...
const ICON_MAP: Record<string, JSX.Element> = {
  "check-circle": <CheckCircle size={26} />,
  award: <Award size={26} />,
  shield: <Shield size={26} />,
  star: <Star size={26} />,
  certificate: <FileBadge size={26} />,
};

const renderIcon = (icon: string) => {
  return ICON_MAP[icon] ?? <CheckCircle size={26} />;
};
...
const CertificateSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/api/certificate`, {
        params: { domainId, courseId },
      })
      .then((res) => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-14 items-center">
          ...
          <div className="w-full lg:w-1/2 space-y-6">
            {data.steps.map((step, index) => (
              <motion.div
                ...
                className="flex gap-5 p-6 rounded-2xl shadow-md"
                style={{ backgroundColor: COLORS.cream }}
              >
                <div
          ...
          >
            ...
            />
            <div
              className="relative z-10 border-4 rounded-xl shadow-2xl bg-white p-2"
              style={{ borderColor: COLORS.gold }}
            >
              <img
                src={`${API_BASE_URL}${data.certificateImage}`}
